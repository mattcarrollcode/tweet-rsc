...
export function TweetView({ tweet }: { tweet: TweetResult }) {
  ...
  const retweetUrl = `https://twitter.com/intent/retweet?tweet_id=${tweet.id_str}`;
  const createdAt = new Date(tweet.created_at);

  const formattedText = formatTweetText(tweet.text, tweet.entities);
  ...
  return (
    <div className="tweet rounded-lg border border-neutral-200 dark:border-neutral-800 px-6 py-4 my-4 bg-white dark:bg-neutral-900 max-w-xl mx-auto">
      ...
      {tweet.photos && (
        <div className="inline-grid grid-cols-1 gap-x-2 gap-y-2 my-2">
          <img
            alt={tweet.text}
      ...
      {tweet.quoted_tweet ? <TweetView tweet={tweet.quoted_tweet} /> : null}
      <a
      ...
      <div className="flex text-sm mt-2">
        ...
        >
          ...
          <span>
            {new Number(
              tweet.conversation_count || tweet.reply_count
            ).toLocaleString("en", {
              notation: "compact",
            })}
          </span>
        ...
        {tweet.retweet_count ? (
          ...
          >
            ...
            <span>
              {new Number(tweet.retweet_count).toLocaleString("en", {
                notation: "compact",
              })}
            </span>
          </a>
        ...
        >
          ...
          <span>
            {new Number(tweet.favorite_count).toLocaleString("en", {
              notation: "compact",
            })}
          </span>
        </a>
