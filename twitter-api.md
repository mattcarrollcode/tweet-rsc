...
This library uses the same Twitter API as the official Twitter web embed widget code
and may change. Current it uses this endpoint:

```
https://cdn.syndication.twimg.com/tweet-result?id=${id}
```

The endpoint returns information on public tweets with the id `${id}`.
The information returned is in JSON format. The strucutre of which is
partially documented in `src/tweet.types.ts`. These types may have
mistakes or the API may shift over time.

Below are examples of JSON returned by the API for Tweets that have
different properties (e.g. images, quote tweets, etc.). This is not
