...
This will be a library that allows you to embed tweets. It uses React's server components to call the Twitter API from a server instead of using Twitter's script to embed the tweet on the page. It currently caches the result from Twitter's API indefinitely.
